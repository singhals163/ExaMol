...
def plot_discovery_trajectory(energies_0, energies_18):
    if not energies_0 or not energies_18:
        print("Missing simulation results in one or both log files.")
        sys.exit(1)

    print(f"Found {len(energies_0)} molecules for Baseline and {len(energies_18)} for Active Learning. Generating trajectory plot...")
    ...
    ax.scatter(df_0.index, df_0['energy'], color='#1f77b4', alpha=0.4, label='Baseline (0 Loops) - Candidates', s=25)
    ax.plot(df_0.index, df_0['running_max'], color='#1f77b4', linewidth=3.5, label='Baseline - Best Discovered', linestyle='--')

    # Plot Active Learning (18 Loops) in Red
    ax.scatter(df_18.index, df_18['energy'], color='#d62728', alpha=0.4, label='AL (18 Loops) - Candidates', s=25)
    ax.plot(df_18.index, df_18['running_max'], color='#d62728', linewidth=3.5, label='AL - Best Discovered')
    ...
    print(f"Success! Trajectory plot saved to: {output_path}")
